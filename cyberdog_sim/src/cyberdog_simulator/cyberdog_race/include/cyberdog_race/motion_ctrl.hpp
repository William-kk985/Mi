...
public:
    ...
    // 俯仰角：⚠ 真机约定【正值=低头、负值=抬头】（舵机方向与直觉相反）
    ...
    // ⚠ 真机走路身高走 303 命令的 pos_des[2]（≈body_height 反馈），
    //   des_roll_pitch_height[2] 参数真机 locomotion 似乎不用（体高反馈不动）
    ...
    // （7671 robot_control_cmd 自动）时 value&0x02 → use_energy_saving_mode=0
    // → LocoGaits 身高 pos_des_(2)=0.13（强制最低）：正常 trot 前进 + 身高 0.13，
    //   只发命令不写参数内存（不会崩 RT 板）
    ...
    // kWalkWave 身高 = des_roll_pitch_height_motion[2] + walk_wave_height
    ...
    // pattern=7→kTrotMedium（普通 trot），body_height→cmd_cur_.pos_des[2]
    ...
    // 能走的通道（robot_control_cmd gait=3）+ pos_des[2]=height，纯命令不发参数→绝对安全！
    // 若真机固件 LocoGaits 读 pos_des[2] → 走路中改 pos_des[2] = 运行时可变身高
    ...
    // control_flags_release.hpp kTrotInOut=56，convex_mpc_motion_gaits.cpp SetTrotInOutParams
    //   offset=(0,9,9,0) 对角同相位=普通 trot，且不清零 vel_cmd_ → 响应快（上限 1.5m/s）
    //   身高 = des_roll_pitch_height_motion[2] → 配合 set_body_params_motion_lcm 走路中动态变身高！
    //   唯一花哨点 trot_in_out_landing_offset[0.05,0.06] 内外八（可改 RT 板 yaml 为 0 消除）
    void set_walk_trot_in_out(float x, float y, float yaw);
    // 运行时改 MotionGaits 身高参数 des_roll_pitch_height_motion（LCM 7668）
    // 与崩 RT 板的 des_roll_pitch_height（LocoGaits 用）是不同参数！
    // 实测 motion 参数被真机读取（0.225 生效）→ 改它 = 走路中动态变身高
    ...
    // 官方留的自定义步态通道！内容 "# Gait Def" 开头 + contact/duration 时序
    //   → 缓存 → 发 gait_id=110(kUserGait) 时 InitUserGaits(true) 用它初始化
    ...
    // 配合 upload_user_gait 先上传自定义步态，再发 110 走路
    ...
    // 步高原始值直通（不 clamp）：排查编码——仿真控制器 (int)%1000*1e-3 解码，疑似期望毫米/打包格式
    ...
    // ✅ 上机验证：低头保持 ~-5° 走满 0.3m（步态读 rpy_des[1]）
    ...
    // ⚠ 官方 pose teleop vel_des=0，能否带速度待上机验证
    ...
    // ★ 命令级方案：201 模式身高读 pos_des[2]（实测精确控身高）+ vel_des 带速度
    //   → 若能走路 = 命令级身高（不写参数内存，绝对安全，实时可变！）
    ...
    // ⚠ 真机 yaml_parameter ROS topic 无订阅者（死通道），但运控有 LCM 参数通道
    ...
    // 步高原始值直通（无 clamp，走 LCM robot_control_cmd 7671）——起步前设一次
    // 仿真控制器解码 (int)%1000*1e-3，疑似期望毫米/打包格式；0.20 米会被 (int) 截成 0
