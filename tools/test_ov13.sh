#!/bin/bash
# cam_id=0 (ov13b10 13MP RGB) 各分辨率测帧+存图
...
for wh in "640 480" "1280 960" "1920 1080"; do
  ...
  sleep 3
  nohup $BIN --ros-args -r __ns:=$NS -p cam_id:=0 -p width:=$W -p height:=$H > /tmp/cc0_$W.log 2>&1 &
  sleep 6
...
if c['img'] is not None:
    ...
    print('[%sx%s] 存图 shape=%s 共%d帧'%(sys.argv[2],c['img'].shape[0],c['img'].shape,c['n']))
else:
    print('[%sx?] NO FRAME 共%d帧'%(sys.argv[2],c['n']))
rclpy.shutdown()
PY
done
pkill -f "cyberdog_race/center_cam" 2>/dev/null
