...
private:
    ...
    float start_odom_yaw_{0.0f};   // 起点 odom 系航向 (高频回正源, 防卡脚偏航反馈慢)
    float start_imu_yaw_{0.0f};    // 起点 external_imu 航向 (石径回正抗抖源)
    float lat_guard_prev_{0.0f};  // SLAM 漂移检测: 上一帧 |latDev|
    ...
    float target_x_{0.0f}, target_y_{0.0f};  // 前进目标点 (点位闭环)
    int   step_time_{0};           // 前进已走帧数 (到位判定指令积分下限)
    float last_x_{0.0f}, last_y_{0.0f};
    float traveled_{0.0f};   // 累计前进位移 (防定位跳变误判 6m)
    int   turn_guard_{0};    // 转弯最小帧数保护 (防 yaw 跳变立即到位)
    int   turn_settle_{0};   // 转向停稳确认帧 (提高转向精度)
    bool  loc_ready_{false}; // global_to_robot 定位是否就绪 (spin 后才有数据)
};
