...
void Stage4Real::run() {
    ...
    switch (state_) {
        ...
        case State::FWD_LINK_1: {   // (2026-08-22 test路线2: 第1轮后前进1.8m)
        ...
        case State::FWD_LINK_2: {   // (2026-08-22 test路线2: 第2轮左转后前进2.5m)
        ...
        case State::TURN_LINK_L: {   // (2026-08-22 test路线2: 最后左转90°立正)
            if (turn_to_yaw(0.0f, +1.5708f)) {
                finish();
            }
            return;
        ...
        case State::TURN_R_OUT: {
...
#endif
#ifdef DEBUG_STAGE4_TEST_ROUTE2
            if (round_count_ == 0) {   // (2026-08-22 test路线2: 第1轮结束右转回正后前进1.8m)
                if (turn_to_yaw(entry_yaw_)) {
                    link_yaw1_ = sensor_.abs_yaw;
                    ++round_count_;
                    fprintf(stderr, "\033[1;35m[S4] 第%d轮完成(前进1.8m衔接)\033[0m\n", round_count_);
                    ref_x_ = last_odom_x_ = sensor_.odom_x;
                    ref_y_ = last_odom_y_ = sensor_.odom_y;
                    travelled_since_ref_ = 0.0f;
                    state_frames_ = 0; sub_state_ = 0;
            ...
            if (round_count_ == 1) {   // (2026-08-22 test路线2: 第2轮结束左转90°→前进2.5m→左转立正)
                if (turn_to_yaw(0.0f, +1.5708f)) {
                    link_yaw2_ = sensor_.abs_yaw;
                    ++round_count_;
                    fprintf(stderr, "\033[1;35m[S4] 第%d轮完成(左转→2.5m→左转立正)\033[0m\n", round_count_);
                    ref_x_ = last_odom_x_ = sensor_.odom_x;
                    ref_y_ = last_odom_y_ = sensor_.odom_y;
                    travelled_since_ref_ = 0.0f;
                    state_frames_ = 0; sub_state_ = 0;
