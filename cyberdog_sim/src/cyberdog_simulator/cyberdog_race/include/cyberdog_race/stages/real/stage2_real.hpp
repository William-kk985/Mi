...
private:
    ...
    void do_step_fix();                   // 步末矫正: 航向→横向, 修完进下一步
    void do_hold();                       // 撞击回点后站稳 1s (替代 0.1m 小步的恢复作用)

    Phase phase_{Phase::STEP};
    bool  done_{false};

    float start_abs_yaw_{0.0f};  // Stage2 开始 SLAM 航向 (DONE 诊断: 量化最终偏转)
    ...
    float step_start_x_{0.0f}, step_start_y_{0.0f};  // 本步起点 (累计漂移闭环)
    ...
    int   turn_guard_{0};       // 停稳复核补转帧计数 (停稳后误差仍大→低速补转)
    float turn_start_x_{0.0f}, turn_start_y_{0.0f};  // 转向起点位置 (ADJUST 顶回基准)
    int   adjust_frames_{0};    // ADJUST 顶回帧计数/超时
    float drift_rate_{0.0f};    // 航向漂移率估计 rad/s (yaw 前馈, 跨步保留; 不加位置积分防 SLAM 放大)
    float last_yaw_err_{0.0f};  // 上一帧航向误差 (漂移率估计用)
    float fwd_integ_{0.0f};     // SLIDE 步前向积分 (闭环自动学习横移角偏)
    float last_dev_fwd_{0.0f};      // 上一帧 dev_fwd (撞击后 odom 假跳检测: 单帧跳>0.1m 沿用上帧)
    float post_impact_guard_{0.0f}; // 撞击回点后保护剩余里程 (前 0.5m 内 vx_lock 限幅±0.05 防左前冲)
    ...
    float fix_lat_integ_{0.0f}; // 横向积分项 (修得更干净)

    // ── 撞击回点后站稳 HOLD (替代 0.1m 小步恢复作用) ──
    int   hold_frames_{0};      // 原地踏步帧计数

    // ── 段中锚点重置 (每 0.7m 重锚, 误差不跨段累积) ──
